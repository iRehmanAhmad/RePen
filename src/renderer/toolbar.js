...
const elements = {
  ...
  mindmapGroupBtn: document.getElementById('mindmapBtn'),
  mindmapGroupPopover: document.getElementById('mindmapGroupPopover'),
  mindmapSubButtons: Array.from(document.querySelectorAll('#mindmapGroupPopover .popover-btn[data-subtool]')),
  mindmapAutoLayoutBtn: document.getElementById('mindmapAutoLayoutBtn'),
...
function currentBrushValue() {
  ...
  if (appState.activeTool === 'shapes') {
    return appState.brushDefaults.shapes;
  }
  if (appState.activeTool === 'eraser') {
    return appState.brushDefaults.eraser;
  }
  if (appState.activeTool === 'mindmap') {
    return appState.brushDefaults.mindmap || appState.brushDefaults.pen;
  }
  return appState.brushDefaults.pen;
}

function updateToolButtons() {
  if (elements.mindmapGroupBtn) {
    const isMindmapActive = !appState.passThrough && appState.activeTool === 'mindmap';
    elements.mindmapGroupBtn.classList.toggle('active', isMindmapActive);
  }
...
if (elements.mindmapGroupBtn) {
  elements.mindmapGroupBtn.addEventListener('click', async () => {
    console.log('[Renderer] mindmapGroupBtn clicked');
    closeAllPopovers();
    await window.appBridge.setTool('mindmap');
    togglePopover(elements.mindmapGroupPopover, elements.mindmapGroupBtn);
  });
}

if (elements.mindmapSubButtons) {
  elements.mindmapSubButtons.forEach(btn => {
...
if (elements.mindmapAutoLayoutBtn) {
  elements.mindmapAutoLayoutBtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    console.log('[Renderer] mindmapAutoLayoutBtn clicked');
    closeAllPopovers();
    await window.appBridge.triggerMindmapAutoLayout();
  });
}
...
if (elements.screenshotButton) {
  elements.screenshotButton.addEventListener('click', async () => {
    await window.appBridge.startSnipMode();
  });
}
...
window.appBridge.getBootstrap().then((bootstrap) => {
  console.log('[Renderer] appBridge:getBootstrap resolved. State keys:', Object.keys(bootstrap.appState || {}));
